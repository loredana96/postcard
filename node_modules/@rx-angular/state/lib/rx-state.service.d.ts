...
export declare type ProjectStateFn<T> = (oldState: T) => Partial<T>;
export declare type ProjectValueFn<T, K extends keyof T> = (oldState: T) => T[K];
export declare type ProjectStateReducer<T, V> = (oldState: T, value: V) => Partial<T>;
export declare type ProjectValueReducer<T, K extends keyof T, V> = (oldState: T, value: V) => T[K];
/**
 * @description
...
export declare class RxState<T extends object> implements OnDestroy, Subscribable<T> {
     ...
     * @return Observable<V>
     */
    select<K extends keyof T, V>(keys: K[], fn: (slice: PickSlice<T, K>) => V, keyCompareMap?: KeyCompareMap<Pick<T, K>>): Observable<V>;
    /**
     * @description
